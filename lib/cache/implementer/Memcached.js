...
const rootPrefix = '../../..',
  cacheHelper = require(rootPrefix + '/lib/cache/helper'),
  ...
  set(key, value, ttl) {
    ...
    return new Promise(function(onResolve) {
      ...
      const callback = function(err) {
        console.log('\n\n-----------MemcacheSetErrorOccured-----------key----', key);
        console.log('-----------MemcacheSetErrorOccured-------1--------err----', err);
        console.log('-----------MemcacheSetErrorOccured-------2--------JSON.stringify(err)----', JSON.stringify(err));
        if (err) {
          const errObj = responseHelper.error({
