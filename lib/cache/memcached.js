...
const MemcachedCacheImplementer = function (isConsistentBehaviour) {
  ...
  // Error handling
  oThis.client.on('issue', function(details){logger.info("Issue with Memcache server. Trying to resolve!")});
  oThis.client.on('failure', function(details){ logger.info( "Server " + details.server + "went down due to: " + details.messages.join( '' ) ) });
  oThis.client.on('reconnecting', function(details){ logger.info( "Total downtime caused by server " + details.server + " :" + details.totalDownTime + "ms")});
