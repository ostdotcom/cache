...
redisCache.prototype = {
  ...
  increment: function (key, byValue) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      if (byValue !== parseInt(byValue, 10) || byValue < 1 || helper.validateCacheValue(byValue) === false) {
        return onResolve(responseHelper.error('l_c_r_i_2', 'Cache value is not an integer'));
      }
      ...
      var callback = function (err, data) {
        if (err) {
          ...
          if (data===false) {
            onResolve(responseHelper.error('l_c_r_i_4', 'Cache key does not exist'));
          } else {
            onResolve(responseHelper.successWithData({response: data}));
          }
        }
      ...
      if (oThis._isConsistentBehaviour) {
        ...
        var incrementValue = function (result) {
          if (result.isSuccess() && result.data.response > 0) {
            ...
            onResolve(responseHelper.error('l_c_m_i_5', 'Cache Key does not exist'));
          }
        };
  ...
  decrement: function (key, byValue) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      if (byValue !== parseInt(byValue, 10) || byValue < 1 || helper.validateCacheValue(byValue) === false) {
        return onResolve(responseHelper.error('l_c_r_d_2', 'Cache value is not an integer'));
      }
      ...
      var callback = function (err, data) {
        if (err) {
          ...
          if (data===false) {
            onResolve(responseHelper.error('l_c_r_d_4', 'Cache key does not exist'));
          } else {
            onResolve(responseHelper.successWithData({response: data}));
          }
        }
      ...
      if (oThis._isConsistentBehaviour) {
        ...
        var decrementValue = function (result) {
          if (result.isSuccess() && result.data.response > 0) {
            ...
            onResolve(responseHelper.error('l_c_m_d_5', 'Cache Key does not exist'));
          }
        };
  ...
  touch: function (key, lifetime) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      if (lifetime !== parseInt(lifetime, 10) || lifetime < 0 || helper.validateCacheValue(lifetime) === false) {
        return onResolve(responseHelper.error('l_c_r_t_2', 'Cache expiry is not a valid number'));
      }
      ...
      var callback = function (err, data) {
        if (err) {
          ...
          if (data===0) {
            onResolve(responseHelper.error('l_c_r_t_4', 'Cache key does not exist'));
          } else {
            onResolve(responseHelper.successWithData({response: data}));
          }
        }
  ...
  delAll: function() {
    ...
    return new Promise(function (onResolve, onReject) {
      client.flushdb(function (err, succeeded) {
        if (err) {
          onResolve(responseHelper.error('l_c_r_t_5', 'Flaush all Cache Keys Failed'));
        } else {
