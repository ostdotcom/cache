...
inMemoryCache.prototype = {
  ...
  set: function (key, value) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      if (record) {
        ...
        record = new Record(value, defaultLifetime);
      }
  ...
  increment: function (key, byValue) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      if (byValue !== parseInt(byValue, 10) || byValue < 1 || helper.validateCacheValue(byValue) === false) {
        return onResolve(responseHelper.error('l_c_im_i_2', 'Cache value is not integer'));
      }
      ...
      if (!record) {
        return onResolve(responseHelper.error('l_c_im_i_3', 'Cache Key does not exists'));
      }
      // Check exiting value is numeric
      var originalValue = record.getValue();
      if (isNaN(originalValue)) {
        return onResolve(responseHelper.error('l_c_im_i_4', 'Cache Key has non numeric value'));
      }
  ...
  decrement: function (key, byValue) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      // NOTE: To support memcached implementation
      if (!record) {
        return onResolve(responseHelper.error('l_c_im_d_3', 'Cache Key does not exists'));
      }
      // Check exiting value is numeric
      var originalValue = record.getValue();
      if (isNaN(originalValue)) {
        return onResolve(responseHelper.error('l_c_im_d_4', 'Cache Key has non numeric value'));
      }

      // NOTE: To support memcached implementation
      byValue = (originalValue < byValue) ? originalValue : byValue;
  ...
  touch: function (key, lifetime) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      if (lifetime !== parseInt(lifetime, 10) || lifetime < 0 || helper.validateCacheValue(lifetime) === false) {
        return onResolve(responseHelper.error('l_c_im_t_2', 'Cache expiry is not valid number'));
      }
